...
import MapGenerator ( randomGrid )
import Data.List ( (\\) )
...
main = do
    [mines, size] <- getArgs
    let
        grid      = randomGrid (read size :: Int) (read size :: Int) (read mines :: Int)
        ...
        playGameGrid showing = do
            ...
            if grid!!row!!col
                ...
                else playGameGrid (uncoverNbhrs count [(row,col)] (row,col) showing)
